...
MODEL_FILES = {
    ...
    "resource": MODEL_DIR / "resource_model.pkl",
}
...
class ResourcePredictionInput(BaseModel):
    power_usage: float = Field(..., ge=0)
    water_usage: float = Field(..., ge=0)
    gas_usage: float = Field(..., ge=0)
    temperature: float = Field(..., ge=-10, le=60)
    humidity: float = Field(..., ge=0, le=100)
...
def predict_resource(input_data: ResourcePredictionInput) -> dict:
    prediction, confidence = predict_from_artifact("resource", input_data.model_dump())
    return {
