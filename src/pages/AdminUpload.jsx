...
const datasets = [
  ...
  {
    key: 'resource_data',
    title: 'Resource Usage Dataset',
    description: 'Columns: state, zone, timestamp/date, power_usage, water_usage, gas_usage, temperature, humidity, usage_score',
  },
];
...
function AdminUpload({ selectedState }) {
  ...
  const handleUpload = async (datasetKey) => {
    ...
    const insertedCount = response?.data?.insertedCount ?? response?.data?.inserted_count ?? response?.data?.inserted ?? null;

    updateDatasetState(datasetKey, {
      status: 'success',
      message: insertedCount ? `Uploaded ${insertedCount} rows successfully.` : 'Upload completed successfully.',
    });
  };
