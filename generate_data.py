...
def generate_resource_dataset(n_rows: int = 5600, random_state: int = 72) -> pd.DataFrame:
    rng = np.random.default_rng(random_state)

    power_usage = np.clip(rng.normal(125, 22, n_rows), 60, 250)
    water_usage = np.clip(rng.normal(50, 13, n_rows), 18, 100)
    gas_usage = np.clip(rng.normal(28, 9, n_rows), 10, 65)
    temperature = np.clip(rng.normal(29, 7, n_rows), 12, 45)
    humidity = np.clip(rng.normal(55, 18, n_rows), 20, 95)

    usage_score = power_usage * 0.18 + water_usage * 0.14 + gas_usage * 0.24 + (temperature - 22) * 0.5 - humidity * 0.05
    anomaly = usage_score > 70

    return pd.DataFrame(
        {
            "power_usage": power_usage.round(2),
            "water_usage": water_usage.round(2),
            "gas_usage": gas_usage.round(2),
...
def generate_all_datasets() -> dict[str, Path]:
    ...
    resource_path = DATA_DIR / "resource_synthetic.csv"
    ...
    generate_resource_dataset().to_csv(resource_path, index=False)
